  ...
  export default{
    ...
    methods: {
      saveRoles(id, index){
        ...
        putRequest("/users/"+id+"/role", this.roles).then(resp=> {
          if (resp.status == 200 && resp.data.status == 'OK') {
            _this.$message({type: resp.data.status, message: resp.data.result});
            _this.loadOneUserById(id, index);
            window.bus.$emit('userManaReload')
          } else {
      ...
      loadRoles(index){
        ...
        getRequest("/users/roles").then(resp=> {
          _this.eploading.splice(index, 1, false)
          if (resp.status == 200) {
            var json = resp.data;
            _this.allRoles = json.result;
          } else {
      ...
      loadUserList(){
        var _this = this;
        getRequest("/users/"+this.keywords).then(resp=> {
          _this.loading = false;
          if (resp.status == 200) {
            var json = resp.data;
            _this.users = json.result;
          } else {
            _this.$message({type: 'error', message: '数据加载失败!'});
          }
        }, resp=> {
