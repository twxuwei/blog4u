  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/categories/', this.cateName).then(resp=> {
          if (resp.status == 200) {
            var json = resp.data;
            _this.$message({type: json.status, message: json.status});
            _this.cateName = '';
            _this.refresh();
          }
          _this.loading = false;
      ...
      handleEdit(index, row){
        ...
        this.$prompt('请输入新名称', '编辑', {
          ...
          if (value == null || value.length == 0) {
            ...
            _this.loading = true;
            putRequest("/categories/"+row.id+"/", value).then(resp=> {
              var json = resp.data;
              _this.$message({
                type: json.status,
                message: json.msg
      ...
      deleteCate(ids){
        ...
        this.loading = true;
        //删除
        deleteRequest("/categories/" + ids).then(resp=> {
          var json = resp.data;
          _this.$message({
            type: json.status,
            message: json.msg
